  ...
  return (
    ...
    >
      ...
      <g id="Group_29109">
        ...
        />
        {symbol && <image x="25" y="25" href={initialIconSrc(symbol)} width="206" height="206" />}
      </g>
    </svg>
  );
});
...
const initialIconSrc = (symbol: string) => {
  ...
  const ext = PNG_ONLY_SYMBOLS.has(key) ? 'png' : 'svg';
  return `/icons/tokens/${key}.${ext}`;
};

function SingleTokenIcon({ symbol, aToken, ...rest }: TokenIconProps) {
  const [src, setSrc] = useState(() => initialIconSrc(symbol));

  useEffect(() => {
    setSrc(initialIconSrc(symbol));
  }, [symbol]);

  const handleError = () => {
    if (src.endsWith('.svg')) {
      setSrc(`/icons/tokens/${symbol.toLowerCase()}.png`);
    } else {
      setSrc('/icons/tokens/default.svg');
    }
  };
  ...
  return (
    <Icon {...rest} sx={{ display: 'flex', position: 'relative', borderRadius: '50%', ...rest.sx }}>
      {aToken ? (
        <ATokenIcon symbol={symbol.toLowerCase()} />
      ) : (
        // eslint-disable-next-line
        <img src={src} onError={handleError} width="100%" height="100%" alt={`${symbol} icon`} />
      )}
    </Icon>
  );
}
